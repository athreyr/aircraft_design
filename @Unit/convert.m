function valout = convertUnit(argin, fromUnit, toUnit)
%convertUnit    Value in new units, based on info from txt file
%   convertUnit(argin, fromUnit, toUnit) returns the value of argin in
%   toUnit, assuming it was already in fromUnit. The conversion factors are
%   read from a txt file. If a direct conversion isn't available, it will
%   attempt to link several pairs of units to get the result.
%   
%   The order of the pairs in the file determines the path that will be
%   taken while linking, so be careful while editing the file. This
%   dependency will be removed in a future release.

parentFolder = fileparts(mfilename('fullpath'));
fileID = fopen(fullfile(parentFolder,'data','unitConversionFactors.txt'));
...
multiplicationFactor = multiplier(fromUnit, toUnit, unitsMatrix, scannedCells{3});

if isempty(multiplicationFactor)
    warning('AD:UnitConversion:badUnits', ...
        ['Conversion from ''%s'' to ''%s'' not available (yet). Please '...
         'type ''edit data/unitConversionFactors.txt'' at the command '...
         'prompt to see list of available units.\nReturning the value '...
         'unchanged'],...
         fromUnit, toUnit)
     valout = argin;
     return
...
valout = multiplicationFactor * argin;

end

function val = multiplier(fromUnit, toUnit, unitsMatrix, multiplierArray)
%multiplier     Mulitiplication factor computed recursively as chain.
%   multiplier(fromUnit, toUnit, unitsMatrix, multiplierArray) returns the
...
%   corresponding list of conversion factors in multiplierArray.
...
for iFrom = 1:numel(fromRows)
    ...
    multiplierFrom = ...
        switchMultiplier(multiplierArray(thisFromRow), thisFromCol, true);
    ...
    for iTo = 1:numel(toRows)
        ...
        multiplierTo = ...
            switchMultiplier(multiplierArray(thisToRow), thisToCol, false);
        ...
        newMultiplierArray = ...
            multiplierArray(setdiff(1:end, [thisFromRow,thisToRow]));
        
        % recurse into this function to calculate the multiplier for
        % converting between new units
        fooVal = ...
            multiplier(newFromUnit, newToUnit, newUnitsMatrix, newMultiplierArray);
