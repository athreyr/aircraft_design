...
%parseInputs    Parse and validate inputs for class constructor
%   [inputPropertiesList, valuesList, OtherInputs] = parseInputs(Default, ...
%       userInputValues);
%   separates the quantities in userInputValues into the cell vector 
%   valuesList (corresponding to each element of the cell vector
%   inputPropertiesList) and the struct OtherInputs (whose fields are
%   different according to the constructor signature used):
%       1. AtmosphericModel(deltaT, |[hMax dh hMin], |[hUb lapseRates], ...
%           |Ref, |SaveUnits)
%       2. AtmosphericModel([hG T], [hGUb isGrad], |Ref, |SaveUnits)
%   
%   The default values corresponding to userInputValues are in the struct
%   Default, which has been created from defaultInputs.
% 
%   See also defaultInputs, AtmosphericModel.AtmosphericModel.

% DON'T USE OtherInputs, LOOKUP PRESSURE/ DENSITY ALTITUDES AND GET
% TEMPERATURE OFFSET FROM THERE ----------------------- !!!!!!!!!!!!!!!!!!!
% SEE HOW IT'S ACTUALLY DONE ON HOT/COLD DAYS

% DO SOMETHING FOR NON-SCALAR deltaT, BECAUSE vertcat WON'T WORK ----- !!!!
...
if isscalar(userInputValues{1}) % deltaT, hData, regionData, Reference, Units
    ...
    hData(numel(hData)+1:3) = nan; % replace empty numbers with nan
...
elseif isnumeric(userInputValues{1}) % hGTData, regionData, Reference, Units
    ...
    OtherInputs.TRef = Reference.T; % SHOULD & SHOULDN'T BE T(1) ---- !!!!
    OtherInputs.pRef = Reference.p;
    OtherInputs.rhoRef = Reference.rho;
